...
class DARTSSearchNetwork(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, steps=4, multiplier=4, stem_multiplier=3):
    ...
    self.register_buffer('alpha_normal', (1e-3 * torch.randn(k, num_ops)).clone().detach().cuda().requires_grad_(True))
    self.register_buffer('alpha_reduce', (1e-3 * torch.randn(k, num_ops)).clone().detach().cuda().requires_grad_(True))
