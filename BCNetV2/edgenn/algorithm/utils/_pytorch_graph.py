...
class GraphPy(object):
    ...
    def get_nodes(self):
        ...
        for v in self.nodes_io.values():
            nodes.append(dict(node_name=v.debugName, input=v.inputs, output_size=v.tensor_size, op=v.kind, attributes=v.attributes))
        return nodes
...
def parse(graph, trace, args=None, omit_useless_nodes=True):
    ...
    for node in graph.nodes():
        if node.kind() == GETATTR_KIND:
            # EdgeNN: we don't need to consider this type of nodes
            continue
